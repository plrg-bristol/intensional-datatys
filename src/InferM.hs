...
data RefinedScheme where
  RScheme :: [Core.Name] -> [Int] -> ConSet -> Type T -> RefinedScheme
  IScheme :: [Core.Name] -> [Int] -> ConSet -> IType T -> RefinedScheme

instance Outputable (RefinedScheme) where
  ppr (RScheme as xs cs t) = hang (text "forall" <+> fsep (map ppr as) <> dot <> text "forall" <+> fsep (map ppr xs) <> dot <> ppr t)
                                2 (hang (text "where") 2 (ppr cs))

  ppr (IScheme as xs cs t) = hang (text "forall" <+> fsep (map ppr as) <> dot <> text "forall" <+> fsep (map ppr xs) <> dot <> ppr t)
                                2 (hang (text "where") 2 (ppr cs))
...
runInferM :: Monad m => InferM m a -> Core.Module -> Context -> m a
...
getTag :: Monad m => InferM m SrcSpan
...
branch e k x d m = InferM $ \mod gamma loc path fresh cs -> do
  ...
  where
    ...
    getVar' gamma =
      case gamma M.!? Core.getName v of
        ...
        -- Scheme from an interface file
        Just (IScheme as xs cs u) -> do
          -- Localise constraints
          ys <- mapM (\x -> (x,) <$> fresh) xs
          emit $ foldr (uncurry rename) cs ys
...
putVar n t m = InferM $ \mod gamma -> unInferM m mod (M.insert n t gamma)

-- Insert many variables
putVars :: Context -> InferM m a -> InferM m a
putVars vs m = InferM $ \mod gamma -> unInferM m mod (M.union vs gamma)
...
emitSubType (Inj x d as) (Inj y _ as') e
  | x /= y                                   =  mapM_ (\(Core.getName -> n) -> emit (insert (Dom x n) (Dom y n) top e empty)) (slice [d])
...
restrict e m = InferM $ \mod gamma loc path fresh cs -> do
  ...
  where
    ...
    restrict' ts cs = fmap (\(Forall as t) -> RScheme as xs cs' t) ts
      where
