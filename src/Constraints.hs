...
dom ks x d = GuardSet (S.map (\k -> Guard $ M.singleton d $ S.singleton (x, k)) (S.fromList ks))
...
-- A faster conjunction that leads to redundancy
quickConj :: GuardSet -> GuardSet -> GuardSet
quickConj (GuardSet gs) (GuardSet gs') = GuardSet $ S.map (\(Guard s, Guard t) -> Guard (M.unionWith S.union s t)) $ S.cartesianProduct gs gs'
